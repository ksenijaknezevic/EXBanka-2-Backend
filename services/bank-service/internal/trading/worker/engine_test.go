...
func newTestEngine(orders trading.OrderRepository, market MarketDataProvider, exchange ExchangeChecker) *Engine {
	return NewEngine(orders, market, &stubFundsManagerEngine{}, exchange, nil, 100*time.Millisecond, nil, nil, nil)
}

// ─── NewEngine ────────────────────────────────────────────────────────────────

func TestNewEngine_NotNil(t *testing.T) {
	e := NewEngine(&stubOrderRepo{}, &stubMarketProvider{}, &stubFundsManagerEngine{}, nil, nil, 0, nil, nil, nil)
	if e == nil {
...
func TestNewEngine_CustomPollInterval(t *testing.T) {
	...
	e := NewEngine(&stubOrderRepo{}, &stubMarketProvider{}, &stubFundsManagerEngine{}, nil, nil, interval, nil, nil, nil)
...
func TestNewEngine_WithTickBus(t *testing.T) {
	bus := NewPriceTickBus()
	e := NewEngine(&stubOrderRepo{}, &stubMarketProvider{}, &stubFundsManagerEngine{}, nil, nil, 0, nil, bus, nil)
	if e.tickBus != bus {
